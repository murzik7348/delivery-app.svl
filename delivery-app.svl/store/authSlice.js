...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    removeAddress: (state, action) => {
      state.addresses = state.addresses.filter((a) => (a.addressId || a.id) !== action.payload);
    },
  ...
  extraReducers: (builder) => {
    ...
    builder
      .addCase(fetchAddresses.fulfilled, (state, action) => {
        const rawAddresses = action.payload ?? [];
        // The backend doesn't return the full address string, 
        // so we derive it from the available fields for the UI to display.
        state.addresses = rawAddresses.map(addr => ({
            ...addr,
