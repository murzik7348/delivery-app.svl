...
struct LiveActivityWidget: Widget {
  var body: some WidgetConfiguration {
    ActivityConfiguration(for: LiveActivityAttributes.self) { context in
      ...
      let rideData = context.state.subtitle?.toRideData()
        
      return DynamicIsland {
        DynamicIslandExpandedRegion(.leading, priority: 1) {
          dynamicIslandExpandedLeading(title: rideData?.driverName ?? "4ABC123", subtitle: rideData?.carModel ?? "Mercedes-Benz")
             ...
             VStack {
               ...
               ZStack {
                 Circle()
                   .fill(
                     LinearGradient(
                       gradient: Gradient(colors: [Color(red: 1.0, green: 0.36, blue: 0.0), Color(red: 1.0, green: 0.18, blue: 0.47)]),
